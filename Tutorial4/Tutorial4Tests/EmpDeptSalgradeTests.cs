...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = emps.Where(e => e.DeptNo == 30).OrderByDescending(emp => emp.Sal).ToList();
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo).Select(emp2 => new {DeptNo = emp2.Key, Count = emp2.Count()}).ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(emp => grades.Where(g => emp.Sal >= g.Losal && emp.Sal <= g.Hisal).Select(g => new {emp.EName, g.Grade}));
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo).Select(SAL => new {DeptNo = SAL.Key, AvgSal = SAL.Average(emp => emp.Sal)}); 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(emp => emp.Sal > emps.Where(e => e.DeptNo == emp.DeptNo).Average(e => e.Sal)).Select(emp => emp.EName);
